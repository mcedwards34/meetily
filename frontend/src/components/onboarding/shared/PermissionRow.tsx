  ...
  return (
    <div
      className={cn(
        ...
        isAuthorized ? 'border-green-300 dark:border-green-800 bg-green-50 dark:bg-green-950' : isDenied ? 'border-red-300 dark:border-red-800 bg-red-50 dark:bg-red-950' : 'bg-card border-border'
      )}
    >
      {/* Left side: Icon + Info */}
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div
          className={cn(
            ...
            isAuthorized ? 'bg-green-100 dark:bg-green-900' : isDenied ? 'bg-red-100 dark:bg-red-900' : 'bg-muted'
          )}
        >
          <div className={cn(isAuthorized ? 'text-green-700 dark:text-green-400' : isDenied ? 'text-red-500 dark:text-red-400' : 'text-muted-foreground')}>{icon}</div>
        </div>
